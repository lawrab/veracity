...
"""
# ruff: noqa: S311,DTZ005  # Random and datetime.now() acceptable for mock data
...
async def get_mock_trending_stories():
    ...
    for i in range(12):
        story = {
            "id": f"story_{i + 1}",
            "title": (
                f"Breaking: Mock Story {i + 1} - "
                f"{random.choice(['Development', 'Update', 'Analysis'])}"
            ),
            "description": (
                "This is a mock story description for testing purposes. "
                "It contains enough text to show how the story card "
                "handles longer descriptions in the frontend interface."
            ),
