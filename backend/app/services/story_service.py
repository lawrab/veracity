...
from app.models.sql_models import Story, Trend, TrustSignal, Correlation
from app.schemas.story import (
    StoryResponse, StoryCreate, TrustScoreHistory, 
    StoryCorrelation, StoryCorrelationsResponse
)
...
class StoryService:
    ...
    async def get_trust_score_history(self, story_id: str) -> Optional[TrustScoreHistory]:
        ...
        if not signals_data:
            return TrustScoreHistory(
                timestamps=[story.created_at],
                scores=[story.trust_score],
                signals=[{}]
            )
        ...
        for ts in timestamps:
            ...
            composite_score = total_weighted_value / total_weight if total_weight > 0 else 0
    ...
    async def get_story_correlations(self, story_id: str) -> Optional[StoryCorrelationsResponse]:
        ...
        # Get correlations
        correlations_query = select(Correlation).where(
            Correlation.story_id == story_id
        ...
        avg_similarity = (
            ...
            if total_correlations > 0 else 0
        )
        ...
        avg_time_to_mainstream = (
            sum(mainstream_times) / len(mainstream_times)
            if mainstream_times else None
        )
    ...
    async def update_trust_score(self, story_id: str, new_score: float, signals: List[Dict[str, Any]]) -> bool:
