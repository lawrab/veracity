...
from app.models.sql_models import Trend, Story, Mention, Source
from app.schemas.trend import TrendResponse, TrendCreate, TrendEvolution, TrendSource
...
class TrendService:
    ...
    async def get_trend_evolution(self, trend_id: str) -> Optional[TrendEvolution]:
        ...
        mentions_query = select(
            ...
            func.date_trunc('hour', Mention.posted_at)
        ).order_by('hour')
    ...
    async def get_trend_sources(self, trend_id: str) -> Optional[List[TrendSource]]:
        ...
        sources_query = select(
            ...
            Source, Mention.source_id == Source.id
        ).where(
            Mention.trend_id == trend_id
        ).group_by(
            Source.platform
        ...
        for platform_info in platform_data:
            ...
            top_sources_query = select(
                ...
                Source.id, Source.username, Source.display_name, Source.credibility_score
            ).order_by(
                func.count(Mention.id).desc()
            ).limit(5)
            ...
            trend_sources.append(TrendSource(
