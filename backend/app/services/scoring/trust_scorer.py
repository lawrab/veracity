...
from app.models.sql_models import Story, Source, TrustSignal, Correlation
...
class TrustScorer:
    ...
    async def calculate_score(self, story: StoryResponse) -> Dict[str, Any]:
        ...
        try:
            ...
            signals["source_credibility"] = await self._calculate_source_credibility(story)
            signals["velocity_pattern"] = await self._analyze_velocity_pattern(story)
            signals["cross_platform_correlation"] = await self._analyze_cross_platform_correlation(story)
            signals["engagement_authenticity"] = await self._analyze_engagement_authenticity(story)
            signals["temporal_consistency"] = await self._analyze_temporal_consistency(story)
            ...
            for signal_type, value in signals.items():
                if value is not None:
                    ...
                    explanations.append(self._generate_signal_explanation(signal_type, value))
            ...
            return {
                ...
                "signals": {
                    k: {
                        ...
                        "contribution": v * self.signal_weights[k] if v is not None else 0
                    }
    ...
    async def _calculate_source_credibility(self, story: StoryResponse) -> Optional[float]:
    ...
    async def _analyze_cross_platform_correlation(self, story: StoryResponse) -> Optional[float]:
    ...
    async def _analyze_engagement_authenticity(self, story: StoryResponse) -> Optional[float]:
    ...
    async def _analyze_temporal_consistency(self, story: StoryResponse) -> Optional[float]:
        ...
        try:
            ...
            time_since_creation = datetime.now(timezone.utc) - story.created_at.replace(tzinfo=timezone.utc)
    ...
    def _generate_signal_explanation(self, signal_type: str, value: Optional[float]) -> str:
    ...
    async def detect_bots(self, posts: List[Dict[str, Any]]) -> Dict[str, Any]:
        ...
        try:
            ...
            bot_probability = min(1.0, len(suspicious_accounts) / max(len(account_stats), 1))
            
            # Boost probability for coordinated indicators
            if coordinated_indicators > 0:
                bot_probability = min(1.0, bot_probability + 0.3 * coordinated_indicators)
    ...
    async def update_with_correlation(self, article: Dict[str, Any], correlation: Dict[str, Any]) -> float:
        """
        ...
        try:
            ...
            logger.info(f"Updated trust score from {current_score} to {updated_score} "
