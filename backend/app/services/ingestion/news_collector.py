...
class NewsCollector:
    ...
    async def collect_rss_feeds(self, feed_urls: Dict[str, str] = None) -> List[Dict[str, Any]]:
        ...
        for source_name, feed_url in feed_urls.items():
            try:
                ...
                for entry in feed.entries[:20]:  # Limit to 20 articles per source
                    article = await self._process_rss_entry(entry, source_name, feed_url)
                    if article:
                        collected_articles.append(article)
                
                logger.info(f"Collected {len([a for a in collected_articles if a['source'] == source_name])} articles from {source_name}")
        ...
        logger.info(f"Total collected: {len(collected_articles)} articles from RSS feeds")
        return collected_articles
    
    async def _process_rss_entry(self, entry: Any, source_name: str, feed_url: str) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            article = {
                ...
                "trust_signals": {
                    ...
                    "is_major_outlet": source_name in ["CNN Top Stories", "BBC News", "Reuters Top News", "AP News"],
                }
            }
    ...
    def _get_source_credibility(self, source_name: str) -> float:
        ...
        high_credibility = ["BBC", "Reuters", "AP News", "NPR", "The Guardian", "Nature", "Science Daily", "NASA"]
        medium_credibility = ["CNN", "Al Jazeera", "TechCrunch", "Ars Technica", "The Verge", "Wired", "Bloomberg", "CNBC"]
    ...
    async def search_news_by_keywords(self, keywords: List[str]) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            if data.get("status") == "ok":
                for article_data in data.get("articles", []):
                    article = await self._process_newsapi_article(article_data, keywords)
                    if article:
                        collected_articles.append(article)
            
            logger.info(f"Collected {len(collected_articles)} articles for keywords: {keywords}")
    ...
    async def _process_newsapi_article(self, article_data: Dict, keywords: List[str]) -> Optional[Dict[str, Any]]:
