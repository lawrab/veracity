...
class EnhancedWebSocketManager:
    ...
    async def connect(
        ...
        try:
            ...
            if settings.REQUIRE_WEBSOCKET_AUTH and not await self._authenticate(auth_token):
            ...
            await self.send_direct(websocket, {
                "type": "connection",
                "status": "connected",
                "channel": channel,
    ...
    async def _heartbeat_handler(self, websocket: WebSocket):
        ...
        while websocket in self.connections:
            try:
                ...
                if (datetime.utcnow() - conn_info.last_heartbeat).seconds > self.HEARTBEAT_TIMEOUT:
    ...
    async def broadcast(self, channel: str, message: dict, exclude: Optional[WebSocket] = None):
        """
