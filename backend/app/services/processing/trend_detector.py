...
class TrendDetector:
    ...
    async def detect_trends(self, posts: List[Dict[str, Any]], time_window_hours: int = 2) -> List[Dict[str, Any]]:
    ...
    def _group_posts_by_time(self, posts: List[Dict[str, Any]], window_minutes: int = 60) -> Dict[str, List[Dict[str, Any]]]:
    ...
    async def _cluster_content(self, posts: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            clustering = DBSCAN(
                eps=0.3,
                min_samples=max(3, len(posts) // 20),
                metric='cosine'
            )
            ...
            for cluster_id in set(cluster_labels):
                ...
                cluster_posts = [
                    posts[i] for i, label in enumerate(cluster_labels) 
                    if label == cluster_id
                ]
                
                if len(cluster_posts) >= self.min_mentions:
                    cluster_info = self._analyze_cluster(cluster_posts, vectorizer, tfidf_matrix, cluster_labels, cluster_id)
                    if cluster_info:
    ...
    def _analyze_cluster(self, cluster_posts: List[Dict[str, Any]], vectorizer, tfidf_matrix, labels, cluster_id) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            # Sentiment analysis
            sentiments = [post.get("sentiment_score", 0) for post in cluster_posts if post.get("sentiment_score") is not None]
            ...
            return {
                ...
                "posts": cluster_posts,
                "first_seen": min(post.get("posted_at", datetime.utcnow()) for post in cluster_posts),
                "last_seen": max(post.get("posted_at", datetime.utcnow()) for post in cluster_posts)
            }
    ...
    def _analyze_keyword_trends(self, posts: List[Dict[str, Any]], time_window_hours: int) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            for keyword, timestamps in keyword_timeline.items():
                if len(timestamps) >= self.min_mentions:
                    trend_info = self._analyze_term_trend(keyword, timestamps, "keyword")
            ...
            for hashtag, timestamps in hashtag_timeline.items():
                if len(timestamps) >= self.min_mentions:
                    trend_info = self._analyze_term_trend(hashtag, timestamps, "hashtag")
                    if trend_info and trend_info["velocity"] >= self.velocity_threshold:
                        trends.append(trend_info)
    ...
    def _analyze_term_trend(self, term: str, timestamps: List[datetime], term_type: str) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            if len(timestamps) >= 4:
                ...
                early_span = (early_timestamps[-1] - early_timestamps[0]).total_seconds() / 3600
                late_span = (late_timestamps[-1] - late_timestamps[0]).total_seconds() / 3600
    ...
    def _analyze_velocity(self, temporal_groups: Dict[str, List[Dict[str, Any]]]) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            for i, (window, velocity) in enumerate(zip(sorted_windows, velocities)):
                if velocity > spike_threshold and velocity >= self.velocity_threshold:
                    ...
                    top_hashtags = [ht for ht, count in Counter(all_hashtags).most_common(5)]
    ...
    async def _analyze_network_patterns(self, posts: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
                    ...
                    for i, comm in enumerate(communities):
                        if len(comm) >= 5:  # Significant community size
                            ...
                            if len(comm_posts) >= self.min_mentions:
                                ...
                                coordination_score = self._calculate_coordination_score(comm_posts)
    ...
    def _combine_trend_signals(self, content_clusters, keyword_trends, velocity_trends, network_trends) -> List[Dict[str, Any]]:
    ...
    def _filter_and_rank_trends(self, trends: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
