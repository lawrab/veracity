...
from app.models.sql_models import Story, TrustSignal, Source
...
class TestTrustScoringIntegration:
    ...
    async def test_complete_trust_scoring_workflow(self, db_session, sample_story):
        ...
        signals_query = select(TrustSignal).where(TrustSignal.story_id == sample_story.id)
    ...
    @pytest.mark.asyncio
    async def test_trust_score_with_different_story_attributes(self, db_session, sample_source):
        ...
        test_scenarios = [
            {
                ...
                "age_hours": 0.5,
                "expected_velocity_score_range": (0.2, 0.4)  # Should be flagged as suspicious
            },
            {
                "name": "organic_story",
            ...
            },
            {
                ...
                "age_hours": 48,
                "expected_velocity_score_range": (0.7, 1.0)  # Should score well for consistency
            }
        ]
        ...
        for scenario in test_scenarios:
            ...
            story = Story(
                ...
                first_seen_at=datetime.now(timezone.utc) - timedelta(hours=scenario["age_hours"]),
                last_updated_at=datetime.now(timezone.utc) - timedelta(minutes=15),
