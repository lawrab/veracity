...
class TestTrustAPI:
    ...
    def mock_trust_score_result(self):
        ...
        return {
            ...
            "signals": {
                "source_credibility": {"value": 0.8, "weight": 0.25, "contribution": 0.2},
                "velocity_pattern": {"value": 0.7, "weight": 0.20, "contribution": 0.14}
            },
...
class TestCalculateTrustScore:
    ...
    def test_calculate_trust_score_success(
        self, mock_websocket, mock_trust_scorer_class, mock_story_service_class, 
        client, sample_story_response, mock_trust_score_result
    ):
    ...
    @patch('app.api.v1.endpoints.trust.StoryService')
    def test_calculate_trust_score_story_not_found(self, mock_story_service_class, client):
        ...
        response = client.post(
            "/api/v1/trust/calculate",
            json={"story_id": "nonexistent-story"}
        )
...
class TestBotDetection:
    ...
    def test_bot_detection_too_many_posts(self, client):
        ...
        large_posts_list = [{"author": f"user{i}", "content": f"post {i}"} for i in range(1001)]
        
        response = client.post(
            "/api/v1/trust/bot-detection",
            json={"posts": large_posts_list}
        )
...
class TestGetCurrentTrustScore:
    ...
    @patch('app.api.v1.endpoints.trust.StoryService')
    def test_get_current_score_success(self, mock_story_service_class, client, sample_story_response):
    ...
    def test_get_current_score_recalculate(
        self, mock_trust_scorer_class, mock_story_service_class, 
        client, sample_story_response, mock_trust_score_result
    ):
        ...
        response = client.get("/api/v1/trust/story/test-story-123/score?recalculate=true")
...
class TestTrustLeaderboard:
    ...
    @patch('app.api.v1.endpoints.trust.StoryService')
    def test_trust_leaderboard_success(self, mock_story_service_class, client, sample_stories_list):
    ...
    @patch('app.api.v1.endpoints.trust.StoryService')
    def test_trust_leaderboard_with_filters(self, mock_story_service_class, client, sample_stories_list):
...
class TestBulkCalculateTrustScores:
    ...
    def test_bulk_calculate_with_errors(
        ...
        mock_story_service.get_story_by_id.side_effect = [
            StoryResponse(
                id="story-1", title="Story 1", description="Desc", category="tech",
                trust_score=50.0, velocity=1.0, geographic_spread={},
                first_seen_at=datetime.now(timezone.utc),
                last_updated_at=datetime.now(timezone.utc),
                created_at=datetime.now(timezone.utc)
            ),
        ...
        mock_trust_scorer.calculate_score.return_value = {
            "score": 0.8, "score_percentage": 80.0, "confidence": 0.9,
            "signals": {}, "explanation": [], "calculated_at": "2024-01-01T12:00:00Z"
        }
        mock_trust_scorer_class.return_value = mock_trust_scorer
