...
class TestTrustScorer:
    ...
    async def test_content_quality_analysis(self, trust_scorer, sample_story):
        ...
        sample_story.description = "This is a comprehensive description with good detail. " * 5
        score = await trust_scorer._analyze_content_quality(sample_story)
        assert score == 0.8
        
        # Test comprehensive content
        sample_story.description = "This is a very comprehensive description with extensive detail. " * 20
        score = await trust_scorer._analyze_content_quality(sample_story)
        assert score == 0.9
    ...
    def test_generate_signal_explanation(self, trust_scorer):
        ...
        explanation = trust_scorer._generate_signal_explanation("source_credibility", 0.75)
        assert "Source credibility: 75.0%" in explanation
        assert "historical accuracy" in explanation
        
        # Test with None value
        explanation = trust_scorer._generate_signal_explanation("velocity_pattern", None)
...
class TestBotDetection:
    ...
    @pytest.mark.asyncio
    async def test_detect_bots_mixed_content(self, trust_scorer, organic_posts, bot_posts):
    ...
    async def test_detect_bots_high_frequency_posting(self, trust_scorer):
        ...
        spam_account = next(
            (acc for acc in result["suspicious_accounts"] if acc["account"] == "spam_bot"),
            None
        )
